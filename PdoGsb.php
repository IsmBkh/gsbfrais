...
/**
 * Retourne les montant unitaire de chaque type de frais 
 */
	// fonction pour recuperer les montantUnitaire 
	public function getMontantUnitaire(){
		$req = "select fraisforfait.montant from fraisforfait order by fraisforfait.id";
		$res = $this->monPdo->query($req);
		$fraisForfaitUnitaire = $res->fetchAll(PDO::FETCH_COLUMN);
		return $fraisForfaitUnitaire;
	}

/**
 * Retourne le montant total des frais pour chaque mois pour chaque user 
 */
	public function getMontantEngage($idVisiteur,$mois){
		...
		$res = $this->monPdo->query($req);
		$montantTotalEngagee = $res->fetch();
		return $montantTotalEngagee;
	}

/**
 * Retourne tous les utilisateurs que nous pourrons afficher dans un tableau 
 */

 	public function getLesUsers(){
	$req = "SELECT id, nom, prenom, dateEmbauche FROM visiteur";
	$res = $this->monPdo->query($req);
	$lesUsers = $res->fetch();
	return $lesUsers;
	}
	...
	public function getInfosGestionnaire($login, $mdp){
		$req = "select gestionnaire.idGestionnaire as id, gestionnaire.nom as nom, gestionnaire.prenom as prenom from gestionnaire 
