...
class etatFraisController extends Controller
{
    ...
    function voirFrais(Request $request){
        if( session('visiteur')!= null){
            ...
            // On recup la requete sql qui calcul le montant total par user dans une variable montantTotalEngagee
            // $TotalEngagee = PdoGsb::getMontantEngage($idVisiteur,$mois)['montant'];

            $vue = view('listefrais')->with('lesMois', $lesMois)
                    // ->with('TotalEngagee', $TotalEngagee)
